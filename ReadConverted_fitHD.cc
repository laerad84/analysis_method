...
int main( int argc, char** argv ){
  Int_t runID = std::atoi( argv[1] );
  std::string ifilename = Form("E42_conv_%d.root",runID);
  std::string ofilename = Form("E42_Fit_%d.root",runID);
  new TGeoManager("TPCGeom","E42 Geometry");
  gGeoManager->Import("~/work/E42_ana/analysis_method/TPC.root");
  TVector3 FieldCenter(0,0,0);
  TVector3 FieldRotation(0,0,6.5*TMath::DegToRad());
  genfit::FieldManager::getInstance()->init( new genfit::VariableField("~/local/hep/E42/E42/data/field/SC_FieldMap.root",FieldCenter, FieldRotation));
  ...
  for( int ievt = 0; ievt < itr->GetEntries(); ievt++){
    ...
    */
    ...
    std::vector<TVector3> plusFit;
    std::vector<TVector3> minusFit;
